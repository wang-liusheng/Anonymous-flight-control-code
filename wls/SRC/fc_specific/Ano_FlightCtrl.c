...
/////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
u8  	search_flag;//测试寻杆标志位
u16   search_pole_flag =0;////寻杆标志位
u16   turnstile_flag = 0;////绕杆标志位
u16   pole_flag=0;///区别杆的标志位
...
/////////////////////////////////////////////////////////
	if(CH_N[AUX1]<-300)
	{
//		flag.flight_mode = ATT_STAB;////纯姿态模式 
		flag.flight_mode = LOC_HOLD;
			...
			//			//		flag.flight_mode = LOC_HOLD;// 定高定点模式
			//					
...
//			flag.unlock_sta = 0;//紧急上锁
//		flag.flight_mode = RETURN_HOME;// gps返航模式
	}
...
//			flag.flight_mode2 = 3//追车（色块）				
//			flag.flight_mode2 = 1;//			
//					search_flag = 2;//1,寻杆  2,前置摄像头追车			
//						pole_flag = 1;  //测试找降落点
...
void ultrasonic()
{
	...
	GPIO_SetBits(GPIOD, GPIO_Pin_5);   //拉高pf9电平	
	Delay_us(30);//持续30us	
	GPIO_ResetBits(GPIOD, GPIO_Pin_5); //拉低pf9电平
...
//					turnstile_flag1++;
					...
					if(opmv.user.data0==0)
					{
						...
						program_ctrl.yaw_pal_dps= 10;///负为右转，正为左转（逆时针）

						OF_rolspeed=0;////roll为负右飞反之为左飞

					}
									else if((opmv.user.data0>=10)&&(opmv.user.data0<=130))
									 {
										 				program_ctrl.yaw_pal_dps= 45;///负为右转，正为左转（逆时针）
...
//																				artificial_yaw += -1.0f;////yaw为负是顺时针，反之逆时针
//																	OF_rolspeed=-15;////roll为负右飞反之为左飞
																}
...
//////////////////////////////////////////////////////////////////////////////		
		...
		if(flag.flight_mode2 == 2)
		{
					...
					{
							program_ctrl.yaw_pal_dps= 10;///负为右转，正为左转（逆时针）
//						artificial_yaw += 0.3f;////yaw为负是顺时针 反之逆时针
						OF_rolspeed=0;////roll为负右飞反之为左飞
...
//										 		OF_rolspeed=15;////roll为负右飞反之为左飞
									...
									///////			
																...
																//												artificial_yaw += 0.8;////yaw为负是顺时针 反之逆时针
																												OF_rolspeed=15;////roll为负右飞反之为左飞
...
//						artificial_yaw += -0.3f;////yaw为负是顺时针 反之逆时针
							...
							{
//									artificial_yaw += 0.1f;////yaw为负是顺时针 反之逆时针
									 program_ctrl.yaw_pal_dps= 8;///负为右转，正为左转（逆时针）
